...
public final class TtoSkip {
    ...
    /** Hard cap on list/map nesting, so a crafted tto stream fails with a clear
     *  {@link IllegalStateException} (the same type this skipper already throws for an
     *  unknown tag) instead of a {@link StackOverflowError}. Real tto data is shallow. */
    ...
    private static void skipList(MessageReader in, int depth) {
        if(depth > MAX_DEPTH)
        ...
        while(!in.eom()) {
            ...
            if(t == T_END)
                break;
            skipTagged(in, t, depth);
        }
    }

    private static void skipTagged(MessageReader in, int t, int depth) {
        switch(t) {
            ...
            case T_TTOL:     skipList(in, depth + 1); break;
            ...
            case T_MAP:      skipList(in, depth + 1); break;
